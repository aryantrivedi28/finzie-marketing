...
export default function ClientRequestForm() {
  ...
  const categoryIcons: Record<ServiceCategory, React.ReactNode> = {
    'Shopify Engine': <ShoppingBag className="w-4 h-4" />,
            ...
            <form onSubmit={handleSubmit} className="p-5 sm:p-6 md:p-8 lg:p-10 space-y-6 md:space-y-8" noValidate>
              ...
              <motion.div variants={fadeInUp} className="pt-4">
                <button
                  type="submit"
                  disabled={loading}
                  className="w-full bg-[#44A194] text-white border-none px-6 sm:px-8 py-4 font-['Jost',sans-serif] text-xs sm:text-[11px] tracking-[0.18em] uppercase cursor-pointer transition-all duration-300 hover:bg-[#38857a] disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-3 group rounded-none"
              ...
              <div className="space-y-3">
                {Object.keys(SERVICE_CATEGORIES).map(category => (
                  <div key={category} className="flex items-center gap-2">
                    <div className="w-2 h-2 rounded-full bg-[#44A194]" />
                    <span className="text-xs text-[#1C2321]">{category}</span>
                  </div>
                ))}
              </div>

              <div className="mt-4 pt-4 border-t border-[rgba(28,35,33,0.08)]">
                <p className="text-[9px] text-[#8a8a82] leading-relaxed">
                  {Object.values(SERVICE_CATEGORIES).flat().length}+ specialized services available
                </p>
              </div>
            </div>
          </motion.aside>
