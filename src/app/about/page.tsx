...
export default function AboutPage() {
  ...
  // Intersection Observer for scroll animations - fixes the pause/fluctuation issue
  useEffect(() => {
    const observerOptions = {
      root: null,
      rootMargin: '0px',
      threshold: 0.15 // Lower threshold for smoother triggering
    }
    ...
    const sections = [heroRef, timelineRef, valuesRef, leadershipRef, finzieRef]
  ...
  const LeaderCard = React.memo(({ initial, role, title, description, gradient }: LeaderCardProps) => (
    <motion.div
      variants={valueCardVariants}
      whileHover="hover"
  ...
  const handleFinzieClick = useCallback(() => {
    ...
    <main className="flex-1 min-h-screen bg-[#F4F0E4] overflow-x-hidden">
      ...
      >
        <div className="max-w-7xl mx-auto">
          <motion.div
            initial="hidden"
            animate={hasAnimated.leadership ? "visible" : "hidden"}
            variants={fadeInUp}
          >
            <div className="inline-flex items-center gap-2.5 text-[10px] tracking-[0.28em] uppercase text-[#44A194] mb-3 md:mb-4">
              <span className="w-6 h-[1px] bg-[#44A194]"></span>
